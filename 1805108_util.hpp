#ifndef _1805108_UTIL_HPP_
#define _1805108_UTIL_HPP_
    ...
    double intersect(Ray &ray, Color &color, int level_of_recursion)
    {
        ...
        for (int i = 0; i < spotLights.size(); i++)
        {
            ...
            if (t_min < t_min_orgininal)
            {
                Vector R = normal * (normal * lightDir) * 2 - lightDir;
                R.normalize();
                double scalingFactor = exp(-lightDistance * lightDistance * spotLights[i].fall_Off_Rate);
                lambert += max(normal * lightDir, 0.0) * scalingFactor;
                phong += max(pow(R * ray.dir, shininess), 0.0) * scalingFactor;

                color = color + this->getColor(initial_Point) * (spotLights[i].color * lambert * this->diffuse);
                color.normalize();

                color = color + (spotLights[i].color * phong * this->specular);
                color.normalize();
            }
        }
...
public:
    ...
    bitmap_image texture1, texture2;
    ...
    {
        int x = (int)((v.x - reference_point.x) / tileWidth);
        int y = (int)((v.y - reference_point.y) / tileWidth);
        int c = (x + y) % 2;
        // int textureMode = 1;
        if (textureMode == 0)
        {
        ...
        else
        {
            if (c == 0)
            {
                ...
                double  tile_x = (v.x - reference_point.x) - x * tileWidth;
                double tile_y = (v.y - reference_point.y) - y * tileWidth;
                int pixel_x = (int)(tile_x * texture1.width() / tileWidth);
                int pixel_y = (int)(tile_y * texture1.height() / tileWidth);
                ...
                if(pixel_x >= texture1.width()){
                    pixel_x = texture1.width() - 1;
                }
                if(pixel_y >= texture1.height()){
                    pixel_y = texture1.height() - 1;
                }

                // pixel_x = pixel_x % texture1.width();
                // pixel_y = pixel_y % texture1.height();

                unsigned char r, g, b;
                texture1.get_pixel(pixel_x, pixel_y, r, g, b);
                
                // Color tex_color(r / 255.0, g / 255.0, b / 255.0);
                // tex_color.normalize();
                double r1 = (r / 255.0) * 0.5 + 0.5*c;
                double g1 = (g / 255.0) * 0.5 + 0.5*c;
                double b1 = (b / 255.0) * 0.5 + 0.5*c;
            ...
            else
            {
                ...
                double tile_x = (v.x - reference_point.x) - x * tileWidth;
                double tile_y = (v.y - reference_point.y) - y * tileWidth;
                int pixel_x = (int)(tile_x * texture2.width() / tileWidth);
                int pixel_y = (int)(tile_y * texture2.height() / tileWidth);

                // pixel_x = pixel_x % texture2.width();
                // pixel_y = pixel_y % texture2.height();

                unsigned char r, g, b;
                texture2.get_pixel(pixel_x, pixel_y, r, g, b);
                if(pixel_x < 0){
                    pixel_x = 0;
                }
                if(pixel_y < 0){
                ...
                if(pixel_x >= texture2.width()){
                    pixel_x = texture2.width() - 1;
                }
                if(pixel_y >= texture2.height()){
                    pixel_y = texture2.height() - 1;
                }
                // Color tex_color(r / 255, g / 255, b / 255);
                // return tex_color;
                // tex_color.normalize();
                double r1 = (r / 255.0) * 0.5 + 0.5*c;
                double g1 = (g / 255.0) * 0.5 + 0.5*c;
